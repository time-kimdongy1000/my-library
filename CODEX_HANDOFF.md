...
1. 댓글 SQL을 변경했다면 Supabase SQL Editor에서 `supabase/comments.sql` 전체를 다시 실행한다.
2. 관리자 비밀번호를 처음 만들거나 바꿀 때만 저장하지 않을 쿼리에서 `private.set_comment_admin_password(...)`를 실행한다.
3. 로컬에서 실제 댓글 작성, 잘못된 비밀번호 거부, 작성자 비밀번호 삭제, 관리자 비밀번호 삭제를 확인한다.
4. 배포 후 운영 주소에서 다시 확인한다.
...
이 검증과 배포가 끝나면 2.0의 핵심 범위가 완료된다.
